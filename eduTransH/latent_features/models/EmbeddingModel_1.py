...
class EmbeddingModel(abc.ABC):
    ...
    def _get_model_loss(self, dataset_iterator):
        ...
        with tf.control_dependencies(dependencies):
            ...
            if self.use_focusE:
                ...
                self.eta = 1
                print(self.eta)
                weights = tf.Print(weights, [weights], message="This is weights: ",summarize=-1)
                
                weights = tf.reduce_mean(weights, 1)
                weights = tf.Print(weights, [weights], message="This is weights1: ",summarize=-1)
                #print(tf.tile(weights, [self.eta]))
                weights_pos = structure_weight + (1 - structure_weight) * (1 - weights)
                #c = tf.constant([self.k,8], tf.int32)
                #weights_pos = tf.tile(weights_pos,c)
                ...
                #scores_pos1 = scores_pos
                scores_pos = tf.transpose(scores_pos)
                scores_pos = scores_pos * weights_pos
                print("printinh")
                print(scores_pos)
                scores_pos = tf.Print(scores_pos, [scores_pos], message="This is scores_pos_1: ",summarize=-1)
                    
            if self.loss.get_state('require_same_size_pos_neg'):
                logger.debug('Requires the same size of postive and negative')
                #scores_pos = tf.transpose(scores_pos)
                #scores_pos = tf.reshape(tf.tile(scores_pos1, [self.eta]), [tf.shape(scores_pos1)[0] * self.eta])
                #score_pos = tf.transpose(scores_pos)
            ...
            for side in corruption_sides:
                ...
                sess = tf.InteractiveSession()
                scores_neg = tf.Print(scores_neg, [scores_neg], message="This is scores_neg: ", summarize=-1)
                if self.use_focusE:
                    ...
                    scores_neg = tf.transpose(scores_neg)
                    scores_neg = scores_neg * weights_neg
                    #scores_neg = scores_neg * weights_pos
                    print("====")
                    print("printing")
                    print(scores_neg.shape)
                    print(scores_neg)
                    scores_neg = tf.Print(scores_neg, [scores_neg], message="This is scores_neg_1: ", summarize=-1)
                
                # Apply the loss function
                # += self.loss.apply(scores_pos, scores_neg)
                loss += self.loss.apply(tf.transpose(scores_pos), tf.transpose(scores_neg))
    ...
    def _training_data_generator(self):
        ...
        print("iteratorrrrrrrrrrrrrrrrrrrrrrrrrrrrrr")
        print(self.batches_count)
            ...
            if isinstance(X, np.ndarray):
                if focusE_numeric_edge_values is not None:
                    ...
                    if focusE_numeric_edge_values.ndim == 1:
                        ...
                        #print("printing edge values")
                        #print(focusE_numeric_edge_values)
                ...
                #n_len = len(focusE_numeric_edge_values)
                #print(n_len)
                #focusE_numeric_edge_values = np.resize(focusE_numeric_edge_values, (self.k,n_len))
                #print(focusE_numeric_edge_values)
